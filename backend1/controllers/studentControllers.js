...
const markAttendance = async (req, res) => {
  try {
    ...
    const subjectCode = req.body.subjectCode || req.body.subject;
    console.log("SUBJECT RECEIVED:", subjectCode);
    ...
    if (!subjectCode) {
      return res.status(400).json({ message: "Subject code missing ❌" });
    }
    ...
    const classStatus = await ClassStatus.findOne();

    if (!classStatus || !classStatus.isActive) {
      return res.status(403).json({
        message: "Class is not started ❌"
      });
    }
    ...
    const faceResponse = await axios.post("http://localhost:5000/face-compare", {
      image1: recievedImage.toString("base64"),
    ...
    if (faceResponse.data.result === "success") {
      ...
      student.subjectAttendance[subjectCode] =
        Number(student.subjectAttendance[subjectCode] || 0) + 1;

      student.markModified("subjectAttendance");
